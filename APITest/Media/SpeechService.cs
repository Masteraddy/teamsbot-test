...
namespace APITest.Media
{
    ...
    public class SpeechService
    {
        ...
        public async Task AppendAudioBuffer(object? sender, AudioMediaBuffer audioBuffer, string callID)
        {
            if (!_isRunning)
            {
                Start();
                await ProcessSpeech(callID);
            }
        ...
        /// </summary>
        private async Task ProcessSpeech(string callID)
        {
            try
            {
                ...
                _recognizer.Recognized += (s, e) =>
                {
                    if (e.Result.Reason == ResultReason.RecognizedSpeech)
                    {
                        ...
                        var transcript = $"{speaker}: {e.Result.Text}\n";
                        if (FileHelper.FileExists(callID + ".txt"))
                        {
                            FileHelper.UpdateFile(callID + ".txt", transcript);
                        }
                        // else
                        // {
                        //     FileHelper.CreateFile(callID + ".txt");
                        //     FileHelper.UpdateFile(callID + ".txt", transcript);
                        // }
